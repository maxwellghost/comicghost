...
- **Continue reading shelf**, **search**, **sort** (title, date added, recently read, unread first, rating)
...
- **Floating magnifier**
- Next-page preloading, auto-hiding or fully hidden chrome, edge progress bar, full screen
...
`unrar` is bundled in the repo and copied into the app at build time — no download or setup needed. 7z, tar, and PDF use frameworks that ship with macOS.
...
- Reading progress, labels, collections, and bookmarks live in a local SwiftData store. Your comics folder is never modified except when you explicitly remove an issue, which moves the file to the Trash.
...
Per-issue and general notes, a pull list for comics you don't own yet, a metadata editor that writes ComicInfo.xml, loose image folder support, an umbrella-folder rework, and working keyboard navigation in the library grid.
